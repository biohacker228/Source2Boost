...
/// кэш-CCD → просадки FPS и статтер даже на мощном железе. Твик сажает cs2.exe на правильные
...
public sealed class Cs2AffinityTweak : ITweak
{
    ...
    public L10n Description { get; } = new(
        "Сажает CS2 на быстрые ядра (P-ядра у Intel-гибрида / первый CCD у Ryzen). Движок Source 2 сам раскладывает главный поток неудачно — уводит на E-ядра или медленный CCD, отсюда просадки и статтер даже на сильном CPU. Применяется при каждом запуске CS2 (игра иногда сбрасывает — переставляем).",
        "Садить CS2 на швидкі ядра (P-ядра у Intel-гібрида / перший CCD у Ryzen). Рушій Source 2 сам розкладає головний потік невдало — тягне на E-ядра або повільний CCD, звідси просадки та статтер навіть на сильному CPU. Застосовується при кожному запуску CS2.",
    ...
    public L10n Impact { get; } = new(
        "+ровность (−статтер)", "+рівність (−статтер)", "+smoothness (−stutter)");
