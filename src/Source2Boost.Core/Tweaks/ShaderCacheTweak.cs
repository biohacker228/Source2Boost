...
/// шейдеров ведёт к более долгой загрузке и статтеру рекомпиляции при следующем запуске.
...
public sealed class ShaderCacheTweak : ITweak
{
    ...
    public L10n Description { get; } = new(
        "Разовая чистка кэшей шейдеров CS2 и GPU по кнопке. БЕЗ ежедневного таймера: постоянное стирание шейдеров вредит — дольше грузится и даёт статтер рекомпиляции. Применять только вручную после крупного обновления/сбоя шейдеров.",
        "Разова чистка кешів шейдерів CS2 та GPU за кнопкою. БЕЗ щоденного таймера: постійне стирання шейдерів шкодить — довше вантажиться та дає статтер рекомпіляції. Застосовувати лише вручну після великого оновлення/збою шейдерів.",
