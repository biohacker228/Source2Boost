...
/// <summary>
/// Периодическая очистка standby-списка памяти (ISLC-подобно) — главный анти-статтер на
/// ограниченной/разнокалиберной RAM. Standby-кэш переполняется и вызывает всплески фреймтайма;
/// периодический сброс убирает микро-статтер. Заменяет прежний вредный shader-таймер.
///
...
public sealed class StandbyCleanTweak : ITweak
{
    ...
    public L10n Title { get; } = new(
        "Очистка standby-памяти (антистаттер)", "Очищення standby-пам'яті (антистаттер)", "Standby memory cleaner (anti-stutter)");
    public L10n Description { get; } = new(
        "Раз в сутки в фоне очищает «зависший» кэш оперативной памяти — он со временем забивается и вызывает всплески кадров и микро-статтер. Полезно даже при большом объёме RAM. Полностью обратимо (удаляет задачу).",
        "Раз на добу у фоні очищає «завислий» кеш оперативної пам'яті — він з часом забивається і спричиняє сплески кадрів та мікро-статтер. Корисно навіть за великого обсягу RAM. Повністю оборотно (видаляє задачу).",
