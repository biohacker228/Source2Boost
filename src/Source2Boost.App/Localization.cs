...
public static class Loc
{
    ...
    private static readonly Dictionary<string, S> _s = new()
    {
        ...
        ["monitor.hint"]    = new("Замер полностью автоматический: программа сама запускает CS2, грузит воркшоп-карту бенчмарка и по окончании прогона берёт её итог. Один и тот же сценарий каждый раз — честное «до/после».",
                                  "Замір повністю автоматичний: програма сама запускає CS2, вантажить воркшоп-карту бенчмарку і по завершенні прогону бере її підсумок. Один і той самий сценарій щоразу — чесне «до/після».",
                                  "Fully automatic: the app launches CS2, loads a workshop benchmark map and takes its result when the run ends. The same scenario every time — a fair before/after."),
        ...
        ["monitor.auto.hint"] = new("Карту нужно один раз подписать в мастерской (кнопка «Открыть карту» → «Подписаться» в Steam). CS2 должен быть закрыт — программа сама запустит его с нужными флагами.",
                                    "Карту треба один раз підписати в майстерні (кнопка «Відкрити карту» → «Підписатися» у Steam). CS2 має бути закритий — програма сама запустить його з потрібними прапорцями.",
                                    "Subscribe to the map once in the Workshop (Open map → Subscribe in Steam). CS2 must be closed — the app starts it with the right flags."),
        ["monitor.auto.title"] = new("Авто-замер (бенч-карта)", "Авто-замір (бенч-карта)", "Auto-benchmark (map)"),
        ["monitor.benchmap.label"] = new("Карта-бенчмарк (ID мастерской):", "Карта-бенчмарк (ID майстерні):", "Benchmark map (workshop ID):"),
        ...
        ["monitor.auto.running"] = new("CS2 уже запущен. Закрой игру — авто-замер сам запустит её с нужной картой и флагами.",
                                       "CS2 вже запущений. Закрий гру — авто-замір сам запустить її з потрібною картою та прапорцями.",
                                       "CS2 is already running. Close it — auto-benchmark will launch it with the right map and flags."),
        ["monitor.auto.loading"] = new("Гружу карту бенчмарка…", "Вантажу карту бенчмарку…", "Loading the benchmark map…"),
        ["monitor.auto.waiting"] = new("Идёт прогон бенчмарка… {0} сек. Дождись, пока карта выведет итог.",
                                       "Триває прогін бенчмарку… {0} с. Дочекайся, поки карта виведе підсумок.",
                                       "Benchmark running… {0}s. Wait for the map to print its result."),
        ["monitor.auto.timeout"] = new("Не дождались итога бенчмарка. Проверь, что карта запустилась и прогон завершился — итог печатается в конце прогона.",
                                       "Не дочекалися підсумку бенчмарку. Перевір, що карта запустилася й прогін завершився — підсумок друкується в кінці прогону.",
                                       "Didn't get a benchmark result. Check the map launched and the run finished — the result prints at the end of the run."),
        ...
        ["monitor.auto.launching"] = new("Запускаю CS2 и демо…", "Запускаю CS2 та демо…", "Launching CS2 and the demo…"),
        ["monitor.auto.launchfail"] = new("Не удалось запустить CS2 через Steam. Запусти игру вручную и попробуй снова.", "Не вдалося запустити CS2 через Steam. Запусти гру вручну та спробуй знову.", "Couldn't launch CS2 via Steam. Start the game manually and try again."),
