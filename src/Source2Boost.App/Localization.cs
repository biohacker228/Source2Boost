...
public static class Loc
{
    ...
    private static readonly Dictionary<string, S> _s = new()
    {
        ...
        ["cs2.shader.title"] = new("Лечение стуттера: сброс шейдеров", "Лікування стуттера: скидання шейдерів", "Fix stutter: reset shaders"),
        ["cs2.shader.desc"] = new("Битый кэш шейдеров CS2/драйвера — частая причина фризов при стрельбе и пике даже на мощном ПК. Разово чистим его (пересоберётся при следующем запуске: первые пару минут догрузка, потом ровно). Делай после крупного обновления или если появились дёргания.",
                                  "Битий кеш шейдерів CS2/драйвера — часта причина фризів при стрільбі та піку навіть на потужному ПК. Разово чистимо (пересобереться при наступному запуску). Роби після великого оновлення або якщо з'явились смикання.",
                                  "A broken CS2/driver shader cache is a common cause of stutter when shooting or peeking, even on strong PCs. This clears it once (rebuilds on next launch: a couple minutes of loading, then smooth). Do it after a big update or if stutter appears."),
