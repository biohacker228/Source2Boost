...
public static class Loc
{
    ...
    private static readonly Dictionary<string, S> _s = new()
    {
        ...
        ["monitor.auto.btn"] = new("Авто-замер по демке", "Авто-замір по демці", "Auto-benchmark (demo)"),
        ["monitor.auto.hint"] = new("Авто-замер прогоняет эталонную демку (одна и та же сцена каждый раз — честное «до/после»). Демку нужно один раз записать; кнопка подскажет как.",
                                    "Авто-замір проганяє еталонну демку (одна й та сама сцена щоразу — чесне «до/після»). Демку треба один раз записати; кнопка підкаже як.",
                                    "Auto-benchmark plays a reference demo (the same scene every time — a fair before/after). The demo must be recorded once; the button explains how."),
        ["monitor.auto.title"] = new("Авто-замер по демке", "Авто-замір по демці", "Auto-benchmark (demo)"),
