...
public static class Loc
{
    ...
    private static readonly Dictionary<string, S> _s = new()
    {
        ...
        ["monitor.auto.hint"] = new("Авто-замер сам запускает CS2 и грузит воршоп-карту бенчмарка (один и тот же сценарий каждый раз — честное «до/после»). Карту нужно один раз подписать в мастерской (кнопка «Подписаться»). CS2 должен быть закрыт — прога запустит его сама с нужными флагами.",
                                    "Авто-замір сам запускає CS2 і вантажить воршоп-карту бенчмарку (один і той самий сценарій щоразу — чесне «до/після»). Карту треба один раз підписати в майстерні (кнопка «Підписатися»). CS2 має бути закритий — програма запустить його сама з потрібними прапорцями.",
                                    "Auto-benchmark launches CS2 and loads a workshop benchmark map itself (the same scenario every time — a fair before/after). Subscribe to the map once in the Workshop (Subscribe button). CS2 must be closed — the app starts it with the right flags."),
        ...
        ["monitor.subscribe"] = new("Подписаться", "Підписатися", "Subscribe"),
