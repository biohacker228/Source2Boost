...
public static class Loc
{
    ...
    private static readonly Dictionary<string, S> _s = new()
    {
        ...
        ["monitor.hint"]    = new("Замер полностью автоматический: прога сама запускает CS2, грузит воршоп-карту бенчмарка и по окончании прогона берёт её итог. Один и тот же сценарий каждый раз — честное «до/после».",
                                  "Замір повністю автоматичний: програма сама запускає CS2, вантажить воршоп-карту бенчмарку і по завершенні прогону бере її підсумок. Один і той самий сценарій щоразу — чесне «до/після».",
        ...
        ["monitor.auto.hint"] = new("Карту нужно один раз подписать в мастерской (кнопка «Открыть карту» → «Подписаться» в Steam). CS2 должен быть закрыт — прога сама запустит его с нужными флагами.",
        ...
        ["monitor.auto.running"] = new("CS2 уже запущен. Закрой игру — авто-замер сам запустит её с нужной картой и флагами. Либо загрузи карту вручную и жми обычный «Замерить».",
                                       "CS2 вже запущений. Закрий гру — авто-замір сам запустить її з потрібною картою. Або завантаж карту вручну та тисни звичайний «Заміряти».",
                                       "CS2 is already running. Close it — auto-benchmark will launch it with the right map and flags. Or load the map manually and use the plain Measure."),
        ...
        ["monitor.auto.timeout"] = new("Не дождались итога бенчмарка. Проверь, что карта запустилась и прогон завершился (итог печатается в конце). Можно увеличить ожидание или снять обычным «Замерить».",
                                       "Не дочекалися підсумку бенчмарку. Перевір, що карта запустилася й прогін завершився (підсумок друкується в кінці).",
                                       "Didn't get a benchmark result. Check the map launched and the run finished (the result prints at the end)."),
